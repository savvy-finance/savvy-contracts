    ...
    IVeSvy
{
    ...
    function setSavvyBooster(
        ISavvyBooster savvyBooster_
    ) external onlyOwner {
        Checker.checkArgument(address(savvyBooster_) != address(0), "zero savvy booster contract address");
    ...
    function setAllowlist(IAllowlist _allowlist) external onlyOwner {
        Checker.checkArgument(address(_allowlist) != address(0), 'zero address');
    ...
    function setGenerationRate(uint256 _generationRate) external onlyOwner {
        Checker.checkArgument(_generationRate != 0, 'generation rate cannot be zero');
    ...
    function setInvVoteThreshold(uint256 _invVoteThreshold) external onlyOwner {
        ...
        Checker.checkArgument(_invVoteThreshold != 0, 'invVoteThreshold cannot be zero');
    ...
    function getStakedSvy(address _addr) external view override returns (uint256) {
    ...
    function stake(uint256 _amount) external override nonReentrant whenNotPaused onlyAllowlisted {
    ...
    function getMaxVeSVYEarnable(address _addr) external view override returns (uint256 maxVeSVYEarnable) {
    ...
    function getVeSVYEarnRatePerSec(address _addr) external view override returns (uint256 veSVYEarnRatePerSec) {
        UserInfo storage user = users[_addr];
    ...
    function unstake(uint256 _amount) external override nonReentrant whenNotPaused {
        ...
        Checker.checkArgument(users[sender].amount > _amount - 1, 'not enough balance');
    ...
    function getVotes(address _account) external view virtual override returns (uint256) {
        ...
        if (veSvyBalance * invVoteThreshold > users[_account].amount * maxCap && isUser(_account)) {
            return veSvyBalance;
        } else {
            return 0;
        }
    ...
    function _updateFactor(
        ...
        savvyBooster.updatePendingRewardsWithVeSvy(_addr, userVeSvyBal, totalVeSvyBal);
    }
