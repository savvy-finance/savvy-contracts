...
contract SavvyPriceFeed is Ownable2StepUpgradeable, ISavvyPriceFeed {
    ...
    function setPriceFeed(
        ...
        Checker.checkArgument(baseToken != address(0), "zero base token address");
        Checker.checkArgument(priceFeed != address(0), "zero price feed address");
        priceFeeds[baseToken] = priceFeed;
    }

    /// @inheritdoc ISavvyPriceFeed
    function updateSvyPriceFeed(
        address newFeed
    ) external onlyOwner override {
    ...
    /// @inheritdoc ISavvyPriceFeed
    function getSavvyTokenPrice() external override view returns (uint256) {
