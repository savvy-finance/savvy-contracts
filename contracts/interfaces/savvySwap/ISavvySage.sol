...
interface ISavvySage is IERC20TokenReceiver {
  ...
  function getTotalUnderlyingBuffered(address baseToken) external view returns (uint256 totalBuffered);
  ...
  function getAvailableFlow(address baseToken) external view returns (uint256 availableFlow);
