...
/// @author Savvy DeFi
contract SavvyPositionManager is ISavvyPositionManager, Initializable, Multicall, Mutex {
    ...
    /// @inheritdoc ISavvyState
    function yieldStrategyManager() external view returns (IYieldStrategyManager) {
    ...
    function initialize(InitializationParams calldata params) external initializer {
        Checker.checkArgument(params.protocolFee <= BPS, Errors.SPM_FEE_EXCEEDS_BPS);
        ...
        _yieldStrategyManager       = IYieldStrategyManager(params.yieldStrategyManager);
        ...
        emit BorrowingLimitUpdated(params.borrowingLimitMaximum, params.borrowingLimitBlocks);
    }
    ...
    function acceptAdmin() external override {
        Checker.checkState(pendingAdmin != address(0), Errors.SPM_ZERO_ADMIN_ADDRESS);
        Checker.checkState(msg.sender == pendingAdmin, Errors.SPM_UNAUTHORIZED_PENDING_ADMIN);
    ...
    /// @inheritdoc ISavvyAdminActions
    function addBaseToken(address baseToken_, BaseTokenConfig calldata config) external override lock {
        _onlyAdmin();
    ...
    function addYieldToken(address yieldToken, YieldTokenConfig calldata config) external override lock {
        ...
        TokenUtils.safeApprove(ITokenAdapter(config.adapter).baseToken(), config.adapter, type(uint256).max);

        TokenUtils.safeApprove(yieldToken, address(_yieldStrategyManager), type(uint256).max);
        TokenUtils.safeApprove(ITokenAdapter(config.adapter).baseToken(), address(_yieldStrategyManager), type(uint256).max);
    ...
    /// @inheritdoc ISavvyAdminActions
    function setBaseTokenEnabled(address baseToken_, bool enabled) external override {
    ...
    /// @inheritdoc ISavvyAdminActions
    function configureRepayLimit(address baseToken_, uint256 maximum, uint256 blocks) external override {
        _onlyAdmin();
    ...
    /// @inheritdoc ISavvyAdminActions
    function configureRepayWithCollateralLimit(address baseToken_, uint256 maximum, uint256 blocks) external override {
        _onlyAdmin();
        _yieldStrategyManager.configureRepayWithCollateralLimit(baseToken_, maximum, blocks);
    ...
    function setSavvySage(address _savvySage) external override {
        _onlyAdmin();
        Checker.checkArgument(_savvySage != address(0), Errors.SPM_ZERO_SAVVY_SAGE_ADDRESS);
    ...
    function setProtocolFeeReceiver(address value) external override {
        _onlyAdmin();
        Checker.checkArgument(value != address(0), Errors.SPM_ZERO_PROTOCOL_FEE_RECEIVER_ADDRESS);
        protocolFeeReceiver = value;
        emit ProtocolFeeReceiverUpdated(value);
    }

    /// @inheritdoc ISavvyAdminActions
    function configureBorrowingLimit(uint256 maximum, uint256 blocks) external override {
        _onlyAdmin();
    ...
    function approveBorrow(address spender, uint256 amount) external override {
        ...
        Account storage account = _accounts[sender];
        account.borrowAllowances[spender] = amount;
        emit ApproveBorrow(sender, spender, amount);
    }
    ...
    function approveWithdraw(address spender, address yieldToken, uint256 shares) external override {
        ...
        emit ApproveWithdraw(sender, spender, yieldToken, shares);
    }
    ...
    function depositYieldToken(
        ...
        Checker.checkArgument(recipient != address(0), Errors.SPM_ZERO_RECIPIENT_ADDRESS);
        _yieldStrategyManager.checkSupportedYieldToken(yieldToken);
    ...
    /// @inheritdoc ISavvyActions
    function borrowCredit(uint256 amount, address recipient) external override lock {
    ...
    /// @inheritdoc ISavvyActions
    function repayWithDebtToken(uint256 amount, address recipient) external override lock returns (uint256) {
        _onlyAllowlisted();
        ...
        int256 debt;
        Checker.checkState((debt = _accounts[recipient].debt) > 0, Errors.SPM_INVALID_DEBT_AMOUNT);
    ...
    function repayWithBaseToken(
        ...
        ) = _yieldStrategyManager.repayWithBaseToken(baseToken_, amount, debt);

        // Decrease the amount of the base token which is globally available to be repaid.
        _yieldStrategyManager.decreaseRepayWithBaseTokenLimiter(baseToken_, actualAmount);

        // Update the recipient's debt.
        _updateDebt(recipient, -SafeCast.toInt256(credit));

        // Transfer the repaid tokens to the savvySage.
        actualAmount = TokenUtils.safeTransferFrom(baseToken_, msg.sender, savvySage, actualAmount);

        // Inform the savvySage that it has received tokens.
        IERC20TokenReceiver(savvySage).onERC20Received(baseToken_, actualAmount);

        emit RepayWithBaseToken(msg.sender, baseToken_, actualAmount, recipient, credit);
    ...
    function repayWithCollateral(
        ...
        address baseToken_ = _yieldStrategyManager.repayWithCollateralCheck(yieldToken);
        ...
        Checker.checkState((unrealizedDebt = _calculateUnrealizedDebt(msg.sender)) > 0, Errors.SPM_INVALID_UNREALIZED_DEBT_AMOUNT);
        ...
        ) = _yieldStrategyManager.repayWithCollateral(yieldToken, address(this), shares, minimumAmountOut, unrealizedDebt);

        // Distribute unlocked credit to depositors.
        _distributeUnlockedCreditDeposited(msg.sender);

        uint256 credit = _normalizeBaseTokensToDebt(baseToken_, amountBaseTokens);
        ...
        _yieldStrategyManager.syncYieldToken(yieldToken, amountYieldTokens, false);

        // Decrease the amount of the base token which is globally available to be repaidWithCollateral.
        _yieldStrategyManager.decreaseRepayWithCollateralLimiter(baseToken_, amountBaseTokens);

        // Transfer the repaid tokens to the savvySage.
        TokenUtils.safeTransfer(baseToken_, savvySage, amountBaseTokens);

        // Inform the savvySage that it has received tokens.
        IERC20TokenReceiver(savvySage).onERC20Received(baseToken_, amountBaseTokens);

        emit RepayWithCollateral(msg.sender, yieldToken, baseToken_, actualShares, credit);
    ...
    function donate(address yieldToken, uint256 amount) external override lock {
        ...
        YieldTokenParams memory _yieldToken = _yieldStrategyManager.getYieldTokenParams(yieldToken);
        uint256 shares = _yieldToken.totalShares - _accounts[msg.sender].balances[yieldToken];

        if (shares > 0) {
            _accounts[msg.sender].lastAccruedWeights[yieldToken] = _yieldStrategyManager.donate(yieldToken, amount, shares);
        }
    ...
    function harvest(address yieldToken, uint256 minimumAmountOut) external override lock {
        ...
        // Inform the savvySage that it has received tokens.
        IERC20TokenReceiver(savvySage).onERC20Received(baseToken_, distributeAmount);
    ...
    function _updateBooster(
        ...
        ISavvyBooster(svyBooster).updatePendingRewardsWithDebt(_user, debtAmount, totalDebtAmount);
    }
    ...
    function _onlyRedlisted() internal {
        address sender = msg.sender;
        Checker.checkState(
            sender == wrapTokenGateway ||
            (
                (!redlistActive && !protocolTokenRequired) ||
                ISavvyRedlist(savvyRedlist).isRedlisted(sender, redlistActive, protocolTokenRequired)
            ),
    ...
    function _onlySentinelOrAdmin() internal view {
        ...
        Checker.checkState(sender == admin || sentinels[sender], Errors.SPM_UNAUTHORIZED_SENTINEL_OR_ADMIN);
    }
    ...
    function _preemptivelyHarvestDeposited(address owner) internal {
        ...
        for (uint256 i = 0; i < depositedTokens.values.length; i++) {
            _yieldStrategyManager.preemptivelyHarvest(depositedTokens.values[i]);
        }
    }
    ...
    function _checkBorrowingLimit(uint256 amount) internal view {
        ...
        Checker.checkState(amount <= limit, Errors.SPM_BORROWING_LIMIT_EXCEEDED);
    }
    ...
    function _depositYieldToken(
        ...
        YieldTokenParams memory yieldTokenParams = _yieldStrategyManager.depositPrepare(yieldToken);
        ...
        yieldTokenParams = _yieldStrategyManager.syncYieldToken(yieldToken, amount, true);
        ...
        Checker.checkState(yieldTokenParams.expectedValue <= maximumExpectedValue, Errors.SPM_EXPECTED_VALUE_EXCEEDED);
    ...
    function _withdraw(
        ...
        uint256 amountYieldTokens = _yieldStrategyManager.convertSharesToYieldTokens(yieldToken, shares);
    ...
    function _distributeUnlockedCreditDeposited(address owner) internal {
        Sets.AddressSet storage depositedTokens = _accounts[owner].depositedTokens;
        for (uint256 i = 0; i < depositedTokens.values.length; i++) {
            _yieldStrategyManager.distributeUnlockedCredit(depositedTokens.values[i]);
        }
    }
    ...
    function _wrap(
        ...
        YieldTokenParams memory yieldTokenParams = _yieldStrategyManager.getYieldTokenParams(yieldToken);

        ITokenAdapter adapter = ITokenAdapter(yieldTokenParams.adapter);
        address baseToken_ = yieldTokenParams.baseToken;

        amount = TokenUtils.safeTransferFrom(baseToken_, msg.sender, address(this), amount);
        uint256 wrappedShares = adapter.wrap(amount, address(this));
        Checker.checkState(wrappedShares >= minimumAmountOut, Errors.SPM_SLIPPAGE_EXCEEDED);
    ...
    function _decreaseBorrowAllowance(address owner, address spender, uint256 amount) internal {
        Account storage account = _accounts[owner];
        account.borrowAllowances[spender] -= amount;
    }
    ...
    function _validateCollateralization(address owner) internal view {
        ...
        Checker.checkState(collateralization >= minimumCollateralization, Errors.SPM_UNDERCOLLATERALIZED);
    }
    ...
    function _totalValue(address owner) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < depositedTokens.values.length; i++) {
            ...
            (
                address baseToken_,
                uint256 amountBaseTokens
            ) = _yieldStrategyManager.convertSharesToBaseTokens(yieldToken, shares);

            totalValue += _normalizeBaseTokensToDebt(baseToken_, amountBaseTokens);
        }
    ...
    function _issueSharesForAmount(
        ...
        uint256 shares = _yieldStrategyManager.issueSharesForAmount(yieldToken, amount);
    ...
    function _calculateUnrealizedDebt(address owner) internal view returns (int256) {
        ...
        for (uint256 i = 0; i < depositedTokens.values.length; i++) {
            ...
            ) = _yieldStrategyManager.calculateUnlockedCredit(yieldToken);
    ...
    function _onlyAllowlisted() internal view {
        ...
        Checker.checkState(tx.origin == sender || IAllowlist(allowlist).isAllowed(sender), Errors.SPM_UNAUTHORIZED_NOT_ALLOWLISTED);
    }
    ...
    function _withdrawBaseToken(
        ...
        TokenUtils.safeTransfer(yieldToken, address(_yieldStrategyManager), amountYieldTokens);
        return _yieldStrategyManager.unwrap(yieldToken, amountYieldTokens, recipient, minimumAmountOut);
    }
    ...
    function _getDebtTokenPrice(
        ...
        uint256 baseTokenAmount = _normalizeDebtTokensToBaseToken(baseToken, uint256(debtAmount));
        return ISavvyPriceFeed(svyPriceFeed).getBaseTokenPrice(baseToken, baseTokenAmount);
    }
    ...
    function _normalizeBaseTokensToDebt(address baseToken_, uint256 amount) internal view returns (uint256) {
        BaseTokenParams memory baseTokenParams = _yieldStrategyManager.getBaseTokenParams(baseToken_);
    ...
    ///
    /// @return The normalized amount.
    function _normalizeDebtTokensToBaseToken(address baseToken_, uint256 amount) internal view returns (uint256) {
        BaseTokenParams memory baseTokenParams = _yieldStrategyManager.getBaseTokenParams(baseToken_);
        return amount / baseTokenParams.conversionFactor;
    }
