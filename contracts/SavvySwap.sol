...
contract SavvySwap is ISavvySwap, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
  ...
  modifier onlySentinelOrAdmin() {
    require (hasRole(SENTINEL, msg.sender) || hasRole(ADMIN, msg.sender), "Unauthorized sentinel or admin");
    _;
  }
  ...
  /// @inheritdoc ISavvySwap
  function deposit(uint256 amount, address owner) external override nonReentrant notPaused {
    _onlyAllowlisted();
    amount = TokenUtils.safeTransferFrom(syntheticToken, msg.sender, address(this), amount);
    _updateAccount(
      UpdateAccountParams({
        owner: owner,
  ...
  /// @inheritdoc ISavvySwap
  function swap(uint256 amount) external override nonReentrant onlySage notPaused {
    ...
    while (state.examineTick != 0) {
      ...
      state.accumulatedWeight = state.accumulatedWeight.add(
        FixedPointMath.rational(state.distributeAmount, state.totalUnswapped)
      );
  ...
  /// @inheritdoc ISavvySwap
  function getUnswappedBalance(address owner) external view override returns (uint256 unswappedBalance) {
  ...
  /// @inheritdoc ISavvySwap
  function getSwappedBalance(address owner) external view override returns (uint256 swappedBalance) {
  ...
  function _updateAccount(UpdateAccountParams memory params) internal {
    ...
    state.totalUnswapped = LiquidityMath.addDelta(state.totalUnswapped, params.unswappedDelta);
    state.unswappedBalance = LiquidityMath.addDelta(state.unswappedBalance, params.unswappedDelta);
    state.swappedBalance = LiquidityMath.addDelta(state.swappedBalance, params.swappedDelta);
  ...
  function _onlyAllowlisted() internal view {
    ...
    require (
      tx.origin == sender ||
      IAllowlist(allowlist).isAllowed(sender),
  ...
  function _normalizeBaseTokensToDebt(uint256 amount) internal view returns (uint256) {
  ...
  function _getswappedBalance(address owner) internal view returns (uint256 swappedBalance) {
    Account storage account = accounts[owner];
