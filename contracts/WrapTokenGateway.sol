...
import "./interfaces/IWrapTokenGateway.sol";
...
contract WrapTokenGateway is IWrapTokenGateway, Ownable2StepUpgradeable {
    ...
    function refreshAllowance(address _savvyPositionManager) external onlyOwner {
        require (
            ISavvyPositionManager(_savvyPositionManager).supportInterface(type(ISavvyAdminActions).interfaceId), 
    ...
    function depositBaseToken(
        ...
        Checker.checkArgument(_amount == msg.value, "unmatched deposit token amount");
        WAVAX.deposit{value: msg.value}();
        TokenUtils.safeApprove(address(WAVAX), _savvyPositionManager, _amount);
        ISavvyPositionManager(_savvyPositionManager).depositBaseToken(_yieldToken, _amount, _recipient, _minimumAmountOut);
    }
    ...
    function withdrawBaseToken(
        ...
        IYieldStrategyManager yieldStrategyManager = ISavvyPositionManager(_savvyPositionManager).yieldStrategyManager();
        ISavvyPositionManager.YieldTokenParams memory params = yieldStrategyManager.getYieldTokenParameters(_yieldToken);
        Checker.checkArgument(params.baseToken == address(WAVAX), "invalid token address");

        uint256 amount = ISavvyPositionManager(_savvyPositionManager).withdrawBaseTokenFrom(msg.sender, _yieldToken, _shares, address(this), _minimumAmountOut);
        _convertWAVAX();
    ...
    function repayWithBaseToken(
        ...
        WAVAX.deposit{value: msg.value}();
        TokenUtils.safeApprove(address(WAVAX), _savvyPositionManager, _amount);
        return ISavvyPositionManager(_savvyPositionManager).repayWithBaseToken(address(WAVAX), _amount, _recipient);
    }
    ...
    function _onlyRedlisted(address _savvyPositionManager) internal {
        bool redlistActive = ISavvyPositionManager(_savvyPositionManager).redlistActive();
        bool protocolTokenRequired = ISavvyPositionManager(_savvyPositionManager).protocolTokenRequired();
        require (
            (!redlistActive && !protocolTokenRequired) ||
            ISavvyRedlist(savvyRedlist).isRedlisted(msg.sender, redlistActive, protocolTokenRequired),
