...
contract InfoAggregator is IInfoAggregator, Ownable2StepUpgradeable {
    ...
    /// @inheritdoc	IInfoAggregator
    function addSavvyPositionManager(address[] calldata _savvyPositionManagers)
        external
        override
        onlyOwner
    {
    ...
    /// @inheritdoc IInfoAggregator
    function addSupportTokens(SupportTokenInfo[] calldata _supportTokens)
        external
        override
        onlyOwner
    {
    ...
    /// @inheritdoc IInfoAggregator
    function getSavvyPositionManagers() external view override returns (address[] memory) {
    ...
    function getWithdrawableAmount(
        ...
        for (uint256 i = 0; i < length; i ++) {
            ISavvyPositionManager savvyPositionManager = ISavvyPositionManager(savvyPositionManagers.at(i));
            (, address[] memory depositedTokens) = savvyPositionManager.accounts(owner_);
            depositedLength += depositedTokens.length;
        } 
        SavvyWithdrawInfo[] memory values = new SavvyWithdrawInfo[](depositedLength);
        uint256 index = 0;

        for (uint256 i = 0; i < length; i ++) {
            ISavvyPositionManager savvyPositionManager = ISavvyPositionManager(savvyPositionManagers.at(i));
            SavvyWithdrawInfo[] memory subValues = InfoAggregatorUtils._getWithdrawableAmount(owner_, savvyPositionManager);
    ...
    function getBorrowableAmount(
        ...
        for (uint256 i = 0; i < length; i ++) {
            ISavvyPositionManager savvyPositionManager = ISavvyPositionManager(savvyPositionManagers.at(i));
            uint256 borrowableAmount = InfoAggregatorUtils._getBorrowableAmount(owner_, savvyPositionManager);
    ...
    function getTotalDebtAmount() public view override returns (int256) {
        ...
        for (uint256 i = 0; i < length; i ++) {
            totalDebt += InfoAggregatorUtils._getTotalDebtAmount(ISavvyPositionManager(savvyPositionManagers.at(i)), svyPriceFeed);
        }
    ...
    function getTotalDepositedAmount() public view override returns (uint256) {
        ...
        for (uint256 i = 0; i < length; i ++) {
            totalAmount += InfoAggregatorUtils._getTotalDepositedAmount(ISavvyPositionManager(savvyPositionManagers.at(i)), svyPriceFeed);
        }
    ...
    function getAvailableCredit() external view override returns (int256) {
        ...
        for (uint256 i = 0; i < length; i ++) {
            totalAmount += InfoAggregatorUtils._getAvailableCreditUSD(ISavvyPositionManager(savvyPositionManagers.at(i)), svyPriceFeed);
        }
    ...
    /// @inheritdoc	ISavvyOverview
    function getAllTokenPrice() external view override returns (TokenPriceData[] memory) {
        uint256 length = savvyPositionManagers.length();
        uint256 tokenAmount = 0;

        for (uint256 i = 0; i < length; i ++) {
            IYieldStrategyManager yieldStrategyManager = ISavvyPositionManager(savvyPositionManagers.at(i)).yieldStrategyManager();
            address[] memory supportedBaseTokens = yieldStrategyManager.getSupportedBaseTokens();
    ...
    function getUserDepositedAmount(address user_)
        public
        view
        returns (uint256)
    {
    ...
    /// @inheritdoc ISavvyUserPortfolio
    function getUserAvailableCredit(address user_)
        external
        view
        returns (int256)
    {
    ...
    function getUserSVYBalance(address user_)
        external
        view
        override
        returns (uint256)
    ...
    /// @inheritdoc ISavvyUserBalance
    function getUserStakedSVYAmount(address user_)
        external
        view
        override
        returns (uint256)
    ...
    /// @inheritdoc ISavvyUserBalance
    function getUserVeSVYBalance(address user_)
        external
        view
        override
        returns (uint256)
    ...
    /// @inheritdoc ISavvyUserBalance
    function getUserClaimableVeSVYAmount(address user_)
        external
        view
        override
        returns (uint256)
    ...
    /// @inheritdoc ISavvyUserBalance
    function getUserClaimableSVYAmount(address user_)
        external
        view
        override
        returns (uint256)
    ...
    /// @inheritdoc ISavvyUserBalance
    function getSVYEarnRate(address user_)
        public
        view
        override
        returns (uint256)
    ...
    /// @inheritdoc ISavvyPositions
    function getTotalDepositedTokenAmount(address user_)
        external
        view
        override
        returns (SavvyPosition[] memory)
    {
        Checker.checkArgument(user_ != address(0), "zero user address");
        ...
        for (uint256 i = 0; i < savvyPositionManagerCnt; i++) {
            ...
            for (uint256 j = 0; j < length; j++) {
                ...
                positions[index].amount += InfoAggregatorUtils._getUserDepositedTokenPrice(
                    user_, 
    ...
    /// @inheritdoc ISavvyPositions
    function getTotalDebtTokenAmount(address user_)
        external
        view
        override
        returns (DebtInfo[] memory)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            debts[i].amount = InfoAggregatorUtils._getUserDebtValueUSD(svyPriceFeed, savvyPositionManager, user_);
        }
    ...
    /// @inheritdoc ISavvyPositions
    function getAvailableDepositTokenAmount(address user_)
        external
        view
        override
        returns (SavvyPosition[] memory)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            amount = InfoAggregatorUtils._getBaseTokenPrice(svyPriceFeed, baseToken, amount);
            positions[i] = SavvyPosition (
                baseToken,
                amount
            );
        }
    ...
    /// @inheritdoc ISavvyPositions
    function getAvailableCreditToken(address user_)
        external
        view
        override
        returns (DebtInfo[] memory)
    {
        ...
        for (uint256 i = 0; i < length; i ++) {
            ISavvyPositionManager savvyPositionManager = ISavvyPositionManager(savvyPositionManagers.at(i));
            int256 debtAmount = InfoAggregatorUtils._getUserDebtValueUSD(svyPriceFeed, savvyPositionManager, user_);
            uint256 depositAmount = InfoAggregatorUtils._getUserDepositedAmount(savvyPositionManager, svyPriceFeed, user_);
            uint256 minCollateralization = savvyPositionManager.minimumCollateralization();
    ...
    function getPoolDeposited(address user_, address poolAddress_)
        external
        view
        override
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < length; i ++) {
            ...
            if (yieldStrategyManager.isSupportedYieldToken(yieldToken)) {
                totalAmount += InfoAggregatorUtils._getUserDepositedTokenPrice(user_, yieldToken, savvyPositionManager, svyPriceFeed);
            }
        }
    ...
    function getPoolUtilization(
        ...
        address yieldToken = poolAddress_;
        IYieldStrategyManager yieldStrategyManager = IYieldStrategyManager(ISavvyPositionManager(savvyPositionManager_).yieldStrategyManager());
        ISavvyState.YieldTokenParams memory yieldTokenParams = yieldStrategyManager.getYieldTokenParameters(yieldToken);

        return (yieldTokenParams.expectedValue, yieldTokenParams.maximumExpectedValue);
    }

    function _addSavvyPositionManagers(address[] memory _savvyPositionManagers) internal {
    ...
    function _addSupportTokens(SupportTokenInfo[] memory _supportTokens)
        internal
    {
        ...
        for (uint256 i = 0; i < length; i ++) {
            Checker.checkState(
                InfoAggregatorUtils._checkSupportTokenExist(supportTokens, _supportTokens[i].yieldToken) == false, 
