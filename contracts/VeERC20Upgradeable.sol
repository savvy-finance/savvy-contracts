...
contract VeERC20Upgradeable is ContextUpgradeable, IVeERC20 {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        Checker.checkArgument(account != address(0), 'ERC20: mint to the zero address');
    ...
    function _burn(address account, uint256 amount) internal virtual {
        Checker.checkArgument(account != address(0), 'ERC20: burn from the zero address');

        _beforeTokenTransfer(account, address(0), amount);

        uint256 accountBalance = _balances[account];
        Checker.checkArgument(accountBalance > amount - 1, 'ERC20: burn amount exceeds balance');
        unchecked {
