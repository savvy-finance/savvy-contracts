...
contract SavvySage is ISavvySage, AccessControlUpgradeable {
    ...
    uint16 constant public FIXED_POINT_SCALAR = 1000;
    ...
    /// @inheritdoc ISavvySage
    function getAvailableFlow(address baseToken)
        external
        view
        override
        returns (uint256)
    ...
    /// @inheritdoc ISavvySage
    function getRegisteredBaseTokens() external view override returns (address[] memory) {
    ...
    /// @inheritdoc ISavvySage
    function getTotalUnderlyingBuffered(address baseToken)
        public
        view
        override
    ...
    function setWeights(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (weightToken != debtToken) {
                ISavvyPositionManager.YieldTokenParams memory params = yieldStrategyManager
                    .getYieldTokenParameters(yieldToken);
                address baseToken = ITokenAdapter(params.adapter)
                    .baseToken();

                Checker.checkArgument(weightToken == baseToken, "invalid weightToken");
            }
    ...
    /// @inheritdoc ISavvySage
    function setSavvySwap(address baseToken, address newSavvySwap) external override onlyAdmin {
        Checker.checkArgument(baseToken == ISavvySwap(newSavvySwap).baseToken(), "invalid base token");
    ...
    function setSlippageRate(uint16 _slippageRate) external override onlyAdmin {
        ...
        Checker.checkArgument(_slippageRate <= 300 && _slippageRate > 0, "invalid slippage rate");
    ...
    function setSavvy(address _savvy) external override onlyAdmin {
        Checker.checkArgument(_savvy != address(0) && savvy != _savvy, "Invalid savvy address");
        if (savvy != address(0)) {
            (, address[] memory depositedTokens) = ISavvyPositionManager(savvy).accounts(address(this));
            Checker.checkState(depositedTokens.length == 0, "Have deposited base tokens");
        }
        ...
        savvy = _savvy;
        yieldStrategyManager = ISavvyPositionManager(savvy).yieldStrategyManager();
    ...
    /// @inheritdoc ISavvySage
    function setAmo(address baseToken, address amo) external override onlyAdmin {
        amos[baseToken] = amo;
        emit SetAmo(baseToken, amo);
    }

    /// @inheritdoc ISavvySage
    function setDivertToAmo(address baseToken, bool divert) external override onlyAdmin {
    ...
    function registerToken(
        ...
        Checker.checkState(yieldStrategyManager.isSupportedBaseToken(baseToken), "base token is not supported");

        // only add to the array if not already contained in it
        Checker.checkState(!registeredBaseTokens.contains(baseToken), "already registered");

        Checker.checkState(ISavvySwap(_savvySwap).baseToken() == baseToken, "invalid base token address");
    ...
    function unregisterToken(
        ...
        Checker.checkState(registeredBaseTokens.contains(_baseToken), "already removed");
        Checker.checkState(ISavvySwap(_savvySwap).baseToken() == _baseToken, "invalid base token address");
        Checker.checkState(savvySwap[_baseToken] == _savvySwap, "invalid savvySwap address");
    ...
    /// @inheritdoc ISavvySage
    function setFlowRate(address baseToken, uint256 _flowRate)
        external
        override
        onlyAdmin
    {
    ...
    function onERC20Received(address baseToken, uint256 amount)
        external
        override
        onlySource
    {
        if (divertToAmo[baseToken]) {
            ...
            // total amount of collateral that the buffer controls in the savvy
            uint256 localBalance = TokenUtils.safeBalanceOf(baseToken, address(this));
            ...
            if (localBalance < flowAvailable[baseToken]) {
                ...
                uint256 swappable = flowAvailable[baseToken] - currentSwapped[baseToken];
    ...
    /// @inheritdoc ISavvySage
    function flushToAmo(address baseToken, uint256 amount) external override onlyKeeper {
    ...
    function withdraw(
        ...
        Checker.checkArgument(amount <= flowAvailable[baseToken], "not enough flowAvailable amount");

        uint256 localBalance = TokenUtils.safeBalanceOf(baseToken, address(this));
        Checker.checkArgument(amount <= localBalance, "not enough local balance");
    ...
    function withdrawFromSavvy(
        ...
        ISavvyPositionManager(savvy).withdrawBaseToken(yieldToken, shares, address(this), minimumAmountOut);
    }
    ...
    function refreshStrategies() public override {
        ...
        Checker.checkState(registeredBaseTokens.length() == supportedBaseTokens.length, "invalid base tokens information");
    ...
    /// @inheritdoc ISavvySage
    function depositFunds(address baseToken, uint256 amount)
        external
        override
        onlyKeeper
    {
        ...
        uint256 localBalance = TokenUtils.safeBalanceOf(baseToken, address(this));
        Checker.checkArgument(localBalance >= amount, "not enough local balance");
        _updateFlow(baseToken);
        ...
        Checker.checkState(localBalance - amount >= currentSwapped[baseToken], "swapped over deposited amount");
    ...
    function _getTotalBuffered(address yieldToken)
        ...
        returns (uint256)
    {
        (uint256 balance,, ) = ISavvyPositionManager(savvy).positions(address(this), yieldToken);
        ISavvyPositionManager.YieldTokenParams memory params = yieldStrategyManager
            .getYieldTokenParameters(yieldToken);
        uint256 tokensPerShare = yieldStrategyManager
            .getBaseTokensPerShare(yieldToken);
    ...
    function _updateFlow(address baseToken) internal returns (uint256) {
        ...
        uint256 marginalFlow = (curTime -
            lastFlowRateUpdate[baseToken]) * flowRate[baseToken];
    ...
    function _savvyDeposit(address token, uint256 amount) internal {
        uint256 minimumAmountOut = yieldStrategyManager.convertBaseTokensToShares(token, amount);
        minimumAmountOut = minimumAmountOut - minimumAmountOut * allowSlippageRate / FIXED_POINT_SCALAR;
    ...
    function _savvyWithdraw(address token, uint256 amountUnderlying) internal {
        ...
        uint256 pricePerShare = yieldStrategyManager.getBaseTokensPerShare(token);
        uint256 wantShares = amountUnderlying * 10**decimals / pricePerShare;
        (uint256 availableShares,, ) = ISavvyPositionManager(savvy).positions(address(this), token);
        if (wantShares > availableShares) {
            wantShares = availableShares;
        }
        // Allow 1% slippage
        uint256 minimumAmountOut = amountUnderlying - amountUnderlying * allowSlippageRate / FIXED_POINT_SCALAR;
        if (wantShares > 0) {
            ISavvyPositionManager(savvy).withdrawBaseToken(token, wantShares, address(this), minimumAmountOut);
        }
    }
    ...
    function _swap(address baseToken) internal {
        ...
        uint256 initialLocalBalance = TokenUtils.safeBalanceOf(baseToken, address(this));
        uint256 want = 0;
    ...
    function _approveTo(address _to, uint256 _approveAmount) internal {
        for (uint256 i = 0; i < registeredBaseTokens.length(); i++) {
            TokenUtils.safeApprove(registeredBaseTokens.at(i), _to, _approveAmount);
        }
