...
contract SavvyRedlist is ISavvyRedlist, Ownable2StepUpgradeable, Mutex {
    ...
    /// @inheritdoc ISavvyRedlist
    function getNFTCollections() external view override returns (address[] memory) {
    ...
    function isRedlisted(
        ...
        for (uint256 i = 1; i < nftCollections.length; i++) {
            ...
            if (_isNFTOwner(account_, nftCollection) && isRedlistNFT(nftCollection)) {
    ...
    function _hasRequiredProtocolTokens(
        ...
        uint256 protocolTokenInWallet = ISavvyToken(protocolToken).balanceOf(account_);
        uint256 protocolTokenStaked = IVeSvy(veProtocolToken).getStakedSvy(account_);
