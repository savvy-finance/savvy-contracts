...
contract SavvySyntheticToken is AccessControl, ReentrancyGuard, ERC20, IERC3156FlashLender {
  ...
  function mint(address recipient, uint256 amount) external onlyMinter ensureUnpaused {
  ...
  function burnFrom(address account, uint256 amount) external {
    Checker.checkState(allowance(account, msg.sender) >= amount, "insufficient allowance");
  ...
  function flashLoan(
    ...
    Checker.checkArgument(amount <= maxFlashLoan(token), "token amount exceeds max flash loan amount");
