...
contract SavvyBooster is 
   ...
   function addPool(
      ...
      Checker.checkArgument(duration_ > 0, "duration must be greater than zero");

      amount_ = TokenUtils.safeTransferFrom(address(svyToken), msg.sender, address(this), amount_);
   ...
   function updatePendingRewardsWithDebt(
      ...
      uint256 pendingRewards = _getClaimableRewards(user_, currentTime);
      uint256 userDebtBalance = _getUserDebt(user_, savvyPositionManager, userDebtSavvy_);
      userDebtPerSavvy[user_][savvyPositionManager] = userDebtSavvy_;
      _updateTotalDebt(savvyPositionManager, totalDebtSavvy_, currentPoolIndex);
   ...
   /// @inheritdoc	ISavvyBooster
   function claimSvyRewards() external override nonReentrant whenNotPaused returns (uint256) {
      ...
      uint256 rewardAmount = Math.min(claimableAmount, currentPool.remainingEmissions);
   ...
   function getSvyEarnRate(
      ...
      uint256 collateralWeightRatio = _getCollateralWeightRatio(user_, currentPoolIndex);
   ...
   function _getClaimableRewards(
      ...
      for (uint256 i = user.lastUpdatePool; i <= currentPoolIndex; i++) {
         ...
         uint256 elapsedTime = _getElaspedTime(user.lastUpdateTime, i, currentTime_);
   ...
   function _getCollateralWeightRatio(
      ...
      return totalCollateralWeight == 0 ? 0 : userCollateralWeight / totalCollateralWeight;
   }
   ...
   function _updateTotalDebt(
      ...
      uint256 totalDebtBalance = lastTotalDebt - lastTotalDebtSavvy + totalDebtSavvy_;
   ...
   function _getElaspedTime(
      ...
      uint256 startTime = Math.max(lastUpdateTime_, pools[poolIndex_].startTime);
      uint256 poolEndTime = pools[poolIndex_].startTime + pools[poolIndex_].duration;
