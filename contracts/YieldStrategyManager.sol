...
contract YieldStrategyManager is IYieldStrategyManager, Mutex, Ownable2StepUpgradeable {
    ...
    mapping(address => Limiters.LinearGrowthLimiter) private _repayWithCollateralLimiters;
    ...
    function setSavvyPositionManager(address savvyPositionManager_) external onlyOwner {
        savvyPositionManager = savvyPositionManager_;
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function getBaseTokensPerShare(address yieldToken) external view returns (uint256) {
        (, uint256 baseTokenAmount) = convertSharesToBaseTokens(yieldToken, 10**_yieldTokens[yieldToken].decimals);
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function isSupportedBaseToken(address baseToken) external view returns (bool) {
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function getBaseTokenParameters(address baseToken)
        external view
        returns (BaseTokenParams memory)
    {
    ...
    function borrowingLimiter() external view returns (Limiters.LinearGrowthLimiter memory) {
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function getRepayLimitInfo(address baseToken)
        external view
        returns (
            uint256 currentLimit,
            uint256 rate,
    ...
    {
        Limiters.LinearGrowthLimiter storage limiter = _repayLimiters[baseToken];
        return (
            limiter.get(),
            limiter.rate,
            limiter.maximum
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function getRepayWithCollateralLimitInfo(address baseToken)
        external view
        returns (
            uint256 currentLimit,
            uint256 rate,
    ...
    {
        Limiters.LinearGrowthLimiter storage limiter = _repayWithCollateralLimiters[baseToken];
        return (
            limiter.get(),
            limiter.rate,
            limiter.maximum
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function decreaseBorrowingLimiter(uint256 amount) external onlySavvyPositionManager {
        _borrowingLimiter.decrease(amount);
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function increaseBorrowingLimiter(uint256 amount) external onlySavvyPositionManager {
        _borrowingLimiter.increase(amount);
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function decreaseRepayWithCollateralLimiter(address baseToken, uint256 amount) external onlySavvyPositionManager {
        _repayWithCollateralLimiters[baseToken].decrease(amount);
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function decreaseRepayWithBaseTokenLimiter(address baseToken, uint256 amount) external onlySavvyPositionManager {
    ...
    function addBaseToken(
        ...
        Checker.checkState(!_supportedBaseTokens.contains(baseToken), "same base token already exists");
        ...
        Checker.checkArgument(
            tokenDecimals < 19 &&
            tokenDecimals < debtTokenDecimals + 1
        ...
        _repayWithCollateralLimiters[baseToken] = Limiters.createLinearGrowthLimiter(
    ...
    function addYieldToken(
        ...
        Checker.checkArgument(config.maximumLoss < BPS + 1, "invalid maximumLoss");
        Checker.checkArgument(config.creditUnlockBlocks > 0, "invalid creditUnlockBlocks");

        Checker.checkState(!_supportedYieldTokens.contains(yieldToken), "same yield token already exists");

        ITokenAdapter adapter = ITokenAdapter(config.adapter);

        Checker.checkState(yieldToken == adapter.token(), "invalid yield token address");
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function setBaseTokenEnabled(address baseToken, bool enabled) external onlySavvyPositionManager {
        _checkSupportedBaseToken(baseToken);
        _baseTokens[baseToken].enabled = enabled;
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function setYieldTokenEnabled(address yieldToken, bool enabled) external onlySavvyPositionManager {
        _checkSupportedYieldToken(yieldToken);
        _yieldTokens[yieldToken].enabled = enabled;
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function configureRepayLimit(address baseToken, uint256 maximum, uint256 blocks) external onlySavvyPositionManager {
        _checkSupportedBaseToken(baseToken);
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function configureRepayWithCollateralLimit(address baseToken, uint256 maximum, uint256 blocks) external onlySavvyPositionManager {
        _checkSupportedBaseToken(baseToken);
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function configureBorrowingLimit(uint256 maximum, uint256 blocks) external onlySavvyPositionManager {
        _borrowingLimiter.update();
        _borrowingLimiter.configure(maximum, blocks);
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function configureCreditUnlockRate(address yieldToken, uint256 blocks) external onlySavvyPositionManager {
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function setTokenAdapter(address yieldToken, address adapter) external onlySavvyPositionManager {
        address oldAdapter = _yieldTokens[yieldToken].adapter;
        Checker.checkState(yieldToken == ITokenAdapter(adapter).token(), "invalid yield token address");
        Checker.checkState(ITokenAdapter(oldAdapter).baseToken() == ITokenAdapter(adapter).baseToken(), "invalid base token address");
        _checkSupportedYieldToken(yieldToken);

        
        TokenUtils.safeApprove(yieldToken, oldAdapter, 0);
        TokenUtils.safeApprove(ITokenAdapter(oldAdapter).baseToken(), oldAdapter, 0);

        TokenUtils.safeApprove(yieldToken, adapter, type(uint256).max);
        TokenUtils.safeApprove(ITokenAdapter(adapter).baseToken(), adapter, type(uint256).max);

        _yieldTokens[yieldToken].adapter = adapter;
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function setMaximumExpectedValue(address yieldToken, uint256 value) external onlySavvyPositionManager {
        _checkSupportedYieldToken(yieldToken);
        _yieldTokens[yieldToken].maximumExpectedValue = value;
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function setMaximumLoss(address yieldToken, uint256 value) external onlySavvyPositionManager {
    ...
    function convertYieldTokensToShares(address yieldToken, uint256 amount) public view returns (uint256) {
        ...
        return amount * yieldTokenParams.totalShares / calculateUnrealizedActiveBalance(yieldToken);
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function convertBaseTokensToShares(address yieldToken, uint256 amount) public view returns (uint256) {
        uint256 amountYieldTokens = convertBaseTokensToYieldToken(yieldToken, amount);
        return convertYieldTokensToShares(yieldToken, amountYieldTokens);
    }

    /// @inheritdoc IYieldStrategyManagerActions
    function snap(address yieldToken) external onlySavvyPositionManager returns (uint256) {
        _checkSupportedYieldToken(yieldToken);

        uint256 expectedValue = convertYieldTokensToBaseToken(yieldToken, _yieldTokens[yieldToken].activeBalance);
    ...
    /// @inheritdoc IYieldStrategyManagerActions
    function preemptivelyHarvest(address yieldToken) public onlySavvyPositionManager {
        ...
        uint256 currentValue = convertYieldTokensToBaseToken(yieldToken, activeBalance);
        ...
        uint256 harvestable = convertBaseTokensToYieldToken(yieldToken, currentValue - expectedValue);
        if (harvestable == 0) {
            return;
        }
    ...
    /// @inheritdoc IYieldStrategyManagerActions
    function donate(address yieldToken, uint256 amount, uint256 shares) external onlySavvyPositionManager returns (uint256) {
        return (_yieldTokens[yieldToken].accruedWeight += amount * FIXED_POINT_SCALAR / shares);
    }
    ...
    function repayWithBaseToken(
        ...
        uint256 maximumAmount = _normalizeDebtTokensToUnderlying(baseToken, uint256(debt));
        ...
        uint256 _currentRepayWithBaseTokenLimit = _repayLimiters[baseToken].get();
        require (actualAmount <= _currentRepayWithBaseTokenLimit, "RepayLimitExceeded");
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function repayLimiters(address baseToken) external view returns (Limiters.LinearGrowthLimiter memory) {
        return _repayLimiters[baseToken];
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function repayWithCollateralLimiters(address baseToken) external view returns (Limiters.LinearGrowthLimiter memory) {
        return _repayWithCollateralLimiters[baseToken];
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function getYieldTokenParams(address yieldToken) public view returns (YieldTokenParams memory) {
        return _yieldTokens[yieldToken];
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function getBaseTokenParams(address baseToken) external view returns (BaseTokenParams memory) {
        return _baseTokens[baseToken];
    }
    ...
    function repayWithCollateral(
        ...
        amountYieldTokens = TokenUtils.safeTransferFrom(yieldToken, msg.sender, address(this), amountYieldTokens);
        uint256 amountBaseTokens = unwrap(yieldToken, amountYieldTokens, recipient, minimumAmountOut);
        ...
        uint256 repayWithCollateralLimit = _repayWithCollateralLimiters[baseToken].get();
        require(amountBaseTokens <= repayWithCollateralLimit, "RepayWithCollateralLimitExceeded");
    ...
    function harvest(
        ...
        uint256 minimumAmountOut, 
        uint256 protocolFee
    ) external onlySavvyPositionManager returns (
        address baseToken,
        ...
        amountBaseTokens = _unwrap(yieldToken, harvestableAmount, savvyPositionManager, minimumAmountOut);
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function currentRepayWithBaseTokenLimit(address baseToken) external view returns (uint256) {
        return _repayLimiters[baseToken].get();
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function currentRepayWithCollateralLimit(address baseToken) external view returns (uint256) {
    ...
    function syncYieldToken(
        ...
        YieldTokenParams memory yieldTokenParams = getYieldTokenParams(yieldToken);

        uint256 amountBaseTokens = convertYieldTokensToBaseToken(yieldToken, amount);
        uint256 updatedActiveBalance   = Math.uoperation(yieldTokenParams.activeBalance, amount, addOperation);
        uint256 updatedExpectedValue   = Math.uoperation(yieldTokenParams.expectedValue, amountBaseTokens, addOperation);
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function convertSharesToBaseTokens(address yieldToken, uint256 shares) public view returns (
        address baseToken,
        uint256 amountBaseTokens
    ) {
        ...
        amountBaseTokens = convertYieldTokensToBaseToken(yieldToken, amountYieldTokens);
    }
    ...
    function convertSharesToYieldTokens(address yieldToken, uint256 shares) public view returns (uint256) {
        ...
        return (shares * calculateUnrealizedActiveBalance(yieldToken)) / totalShares;
    }

    /// @inheritdoc IYieldStrategyManagerStates
    function convertYieldTokensToBaseToken(address yieldToken, uint256 amount) public view returns (uint256) {
        YieldTokenParams storage yieldTokenParams = _yieldTokens[yieldToken];
        ITokenAdapter adapter = ITokenAdapter(yieldTokenParams.adapter);
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function calculateUnrealizedActiveBalance(address yieldToken) public view returns (uint256) {
        YieldTokenParams storage yieldTokenParams = _yieldTokens[yieldToken];
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function convertBaseTokensToYieldToken(address yieldToken, uint256 amount) public view returns (uint256) {
        YieldTokenParams storage yieldTokenParams = _yieldTokens[yieldToken];
        ITokenAdapter adapter = ITokenAdapter(yieldTokenParams.adapter);
    ...
    /// @inheritdoc IYieldStrategyManagerStates
    function calculateUnlockedCredit(address yieldToken) public view returns (
        uint256 currentAccruedWeight,
        uint256 unlockedCredit
    ) {
        YieldTokenParams storage yieldTokenParams = _yieldTokens[yieldToken];
        ...
        currentAccruedWeight += unlockedCredit > 0
                ? unlockedCredit * FIXED_POINT_SCALAR / yieldTokenParams.totalShares
                : 0;
    }
    ...
    function _unwrap(
        ...
        amount = TokenUtils.safeTransferFrom(yieldToken, msg.sender, address(this), amount);
    ...
    function _normalizeBaseTokensToDebt(address baseToken, uint256 amount) internal view returns (uint256) {
    ...
    ///
    /// @return The normalized amount.
    function _normalizeDebtTokensToUnderlying(address baseToken, uint256 amount) internal view returns (uint256) {
    ...
    function _loss(address yieldToken) internal view returns (uint256) {
        ...
        uint256 amountBaseTokens = convertYieldTokensToBaseToken(yieldToken, yieldTokenParams.activeBalance);
        uint256 expectedUnderlyingValue = yieldTokenParams.expectedValue;

        if (amountBaseTokens == 0) {
            return 1;
        } else {
            return expectedUnderlyingValue > amountBaseTokens
            ? ((expectedUnderlyingValue - amountBaseTokens) * BPS) / expectedUnderlyingValue
            : 0;
        }
        
    }

    function _preemptivelyHarvest(address yieldToken, uint256 harvestable) internal {
